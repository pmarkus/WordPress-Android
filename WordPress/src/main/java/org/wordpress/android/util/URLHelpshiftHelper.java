...
public class URLHelpshiftHelper {
    ...
    private static URLHelpshiftHelper mInstance = null;
    ...
    public enum URLTag {
        ...
        private URLTag(final String stringValue) {
            mStringValue = stringValue;
        }
        ...
        public static String[] toString(URLTag[] tags) {
            if (tags == null) {
                return null;
            }
    ...
    private URLHelpshiftHelper() {
    }

    public static synchronized URLHelpshiftHelper getInstance() {
        if (mInstance == null) {
            mInstance = new URLHelpshiftHelper();
        }
        return mInstance;
    }
     ...
     */
    public void showConversation(Activity activity, URLTag origin) {
        if (origin == null) {
            origin = URLTag.ORIGIN_UNKNOWN;
        }
     ...
     */
    public void showFAQ(Activity activity, URLTag origin) {
        if (origin == null) {
            origin = URLTag.ORIGIN_UNKNOWN;
        }
        // track origin and helpshift screen in analytics
    ...
    public void setTags(URLTag[] tags) {
        mMetadata.put(Helpshift.HSTagsKey, URLTag.toString(tags));
    }

    public void addTags(URLTag[] tags) {
        String[] oldTags = (String[]) mMetadata.get(Helpshift.HSTagsKey);
        // Concatenate arrays
        mMetadata.put(Helpshift.HSTagsKey, ArrayUtils.addAll(oldTags, URLTag.toString(tags)));
    }
